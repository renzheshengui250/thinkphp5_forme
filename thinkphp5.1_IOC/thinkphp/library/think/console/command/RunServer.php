...
class RunServer extends Command
{
    public function configure()
    {
        $this->setName('run')
            ->addOption('host', 'H', Option::VALUE_OPTIONAL,
                'The host to service the application on', '127.0.0.1')
            ->addOption('port', 'p', Option::VALUE_OPTIONAL,
                'The port to service the application on', 8000)
    ...
    public function execute(Input $input, Output $output)
    {
        ...
        $output->writeln(sprintf('ThinkPHP Development service is started On <http://%s:%s/>', $host, $port));
